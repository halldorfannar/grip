...
The lib folder in the Ansel SDK contains the import libraries for 32-bit and 64-bit architecture DLLs. Note that we do 
not offer Ansel SDK for static linking. Based on the targeted platform architecture you must link in the corresponding 
import library.
...
-----------------------------
!!!TODO!!! We should update the functional spec and refer readers to that instead - include with SDK.
### 3.1 ACTIVATING AND DEACTIVATING ANSEL
Players can start/stop Ansel session by pressing ALT+F2.
> **NOTE:** If we expose startSession trigger via SDK then games can also expose their own methods to enable Ansel 
(like depressing left stick on gamepad).
